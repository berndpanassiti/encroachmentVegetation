...
Appendix_S4 = ggplot(plot_data_reduziert, aes(x = AlmLabel, y = Gesamtartenzahl, fill = Verbuschung)) +
  geom_boxplot() +
  theme_minimal() +
  labs(
    title = "",
...
# Boxplots for indicator values


```{r}
zeigerwerte_mittel <- readxl::read_excel("data/rawdata/Zeigerwerte_mittel_hl_sl.xlsx")
zeigerwerte_box <- zeigerwerte_mittel
veg_Verbuschung <- data.frame(rownames(veg_layer), plot_data[, 10])
colnames(veg_Verbuschung) = c("Plot_ID", "Verbuschung")
zeigerwerte_box <- merge(zeigerwerte_box, veg_Verbuschung, by = "Plot_ID", all.x = TRUE)
zeigerwerte_box$Verbuschung <- as.factor(zeigerwerte_box$Verbuschung) 

zeigerwerte_box$Alm <- factor(zeigerwerte_box$Alm , levels = c("Saletalm", "Engertalm",
                                                   "Bindalm","Königsbachalm",
                                                   "Halsalm","Königsbachalm 2",
                                                   "Mittereisalm","Königsbergalm"))


zeiger_box_long <- zeigerwerte_box %>%
  pivot_longer(
    cols = c(Lichtzahl, Temperaturzahl, Feuchtezahl, Reaktionszahl, Naehrstoffzahl),
    names_to = "Zeigerwert",
    values_to = "Wert"
  )
```
...
```{r}
ggplot(zeiger_box_long, aes(x = Alm, y = Wert, fill = Verbuschung)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +  # Boxplots nebeneinander
  facet_wrap(~ Zeigerwert, scales = "fixed") +  # Eine Facette pro Zeigerwert
  labs(
    title = "Ellenberg-Zeigerwerte für verschiedene Almen und Verbuschung",
    x = "Alm",
    y = "Zeigerwert"
  ) +
  scale_fill_manual(values = c("unverbuscht" = "#4CAF50", "verbuscht" = "#FF6347")) +  # Farben für Verbuschung
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5)) +
  theme_minimal() +
  theme(
  axis.text.x = element_text(size = rel(1.2), angle = 45, hjust = 1),
  axis.text.y = element_text(size = rel(1.2)),
  strip.text = element_text(size = rel(1.2), face = "bold", margin = margin(b = 14)),
