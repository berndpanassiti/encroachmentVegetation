...
### Forest association and biotope
```{r}
df <- traits %>%
...
df$forestAssociation <- factor(df$forestAssociation, 
                               levels = c("1.1", "1.2", "2.1","2.2","Others"))


# combine tables 34,35 if presence then 1, otherwise 0
df = df %>% dplyr::mutate(
   tables34_35 = case_when(
      !is.na(table34) & !is.na(table35) ~ 1,  # both present → 1
      !is.na(table34) ~ 1,
      !is.na(table35) ~ 1,
      TRUE ~ NA_real_    # neither present
    ))

df$tables34_35[is.na(df$tables34_35)] <- 0

df = df %>% dplyr::mutate(
   Others = case_when(
      is.na(table33) &  is.na(table34) & is.na(table35) ~ 1, 
      TRUE ~ NA_real_    # neither present
    ))

df$Others[is.na(df$Others)] <- 0

df$table33 = ifelse(df$table33 > 0, 1, 0)

df$table33[is.na(df$table33)] <- 0



# filter data for only species presences
df %<>% dplyr::filter(cover>0)


traitsBiotopeForest = df
```


### weight values
```{r}
traits_weighted <- traitsBiotopeForest %>% 
  dplyr::mutate(mowingTolerance_weighted = cover*mowingTolerance, # jf NA then NA
  ...
  dplyr::select(plot_id,species, mowingTolerance_weighted,
                grazingTolerance_weighted,tramplingTolerance_weighted,
...
save(
 ...
 waldarten,rl_bayern,bioflorBiotop,traitsSummaryPlot,traits,traitsBiotopeForest,
