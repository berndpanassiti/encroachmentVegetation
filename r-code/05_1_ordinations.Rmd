...
#Install packages
p_load(tidyverse,dplyr,vegan,ggplot2,magrittr,multcompView)
```
...
nmds_points <- as.data.frame(scores(nmds, display = "sites"))
nmds_points$Site <- rownames(nmds_points)
veg_Verbuschung <- data.frame(rownames(veg_layer), df2_sorted[, 10])
...
Appendix_S6 = ggplot() +
  ...
  scale_size_continuous("Encroachment gradient (%):",
                        range = c(3, 10)) +
